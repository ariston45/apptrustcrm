...
class DataController extends Controller
{
  ...
  // }
	public function sourceDataCustomer(Request $request)
	{
		$colect_data = Cst_customer::join('cst_institutions', 'cst_customers.cst_institution','=', 'cst_institutions.ins_id')
		->leftjoin('cst_locations','cst_customers.cst_id','=','cst_locations.loc_cst_id')
		->where('loc_represent','INSTITUTION')
		->orWhere('loc_represent',null)
		->select('cst_id','ins_name','cst_name', 'cst_string_id', 'cst_string_id', 'cst_customers.created_at','cst_business_field','loc_city')
		->get();
		$num = 1;
		return DataTables::of($colect_data)
		->addIndexColumn()
		...
		->addColumn('menu', function ($colect_data) {
			...
			<a class="dropdown-item" href="#"><i class="ri-run-line" style="margin-right:6px;"></i>Activities</a>
	...
	public function sourceDataLeads(Request $request)
	{
		...
		if ($user->level == 'ADM' || $user->level == 'MGR.PAS' || $user->level == 'AGM') {
			$lead_user_head = Prs_salesperson::where('slm_rules','head')->where('slm_user',$user->id)->select('slm_lead')->get()->toArray();
			$lead_user_member = Prs_salesperson::where('slm_rules','member')->where('slm_user',$user->id)->select('slm_lead')->get()->toArray();
			...
			->leftJoin(DB::raw('(select slm_lead,slm_user,name from prs_salespersons join users on prs_salespersons.slm_user = users.id where slm_rules="head") salesperson'),
			function($join){
		...
		->addColumn('menu', function ($lead_data) {
			...
			<a class="dropdown-item" href="#"><i class="ri-user-add-line" style="margin-right:6px;"></i>Update Status</a>
      </div>
			</div>
			';
		})
