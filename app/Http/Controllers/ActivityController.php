...
class ActivityController extends Controller
{
	...
	public function viewActivity(Request $request)
	{
		...
		if (checkRule(array('ADM','AGM','MGR.PAS'))) {
			...
			$act_access = Act_activity_access::whereIn('acs_user_id',$user->id)->select('acs_act_id')->get();
			$act_idr = array();
		...
		$customer_all = Cst_institution::select('ins_id','ins_name')
		->get();
	...
	public function viewActivityDetail(Request $request)
	{
		...
		$user = Auth::user();
		$all_user = User::join('user_structures', 'users.id', '=', 'user_structures.usr_user_id')
		->leftJoin('user_teams', 'user_structures.usr_team_id', '=', 'user_teams.uts_id')
		->select('id', 'name', 'level', 'uts_team_name')
		->whereNotIn('level', ['ADM', 'AGM'])
		->get();
		...
		->select('act_cst','act_subcst','lds_id','lds_title','aat_type_name','aat_type_code','aat_custom_class_2','aat_icon','act_run_status','act_todo_describe','act_todo_result','act_task_times',
		...
		->first();
		$activity_type = Act_activity_type::get();
		$customer = Cst_institution::where('ins_id',$main_activity->act_cst)
		->select('ins_name')
		->first();
		$subcustomer = Cst_customer::where('cst_id', $main_activity->act_subcst)
		->select('cst_name')
		->first();
		// dd($subcustomer);
		// die();
		...
		// echo $cst_person_all;
		// die();
		return view('contents.page_activity.activity_detail',compact('id_activity','customer','subcustomer','main_activity','team','date_created','date_due','user_created',
	...
	public function sourceDataActivityCalender(Request $request)
	{
		...
		if (checkRule(array('ADM','AGM','MGR.PAS'))) {
			...
			->where('act_label_category', 'ACTIVITY')
			->select('act_id','ins_name','aat_type_button','aat_custom_class_1','act_activities.act_task_times_due','act_todo_result','aat_custom_style_1','aat_type_code',
			'users.name as assign','aat_type_name','act_subcst')
			->get();
		}elseif(checkRule(array('MGR'))) {
			...
			->whereBetween('act_task_times_due',[$start,$end])
			->where('act_label_category', 'ACTIVITY')
			->select('act_id','aat_type_button','aat_custom_class_1','act_activities.act_task_times_due','act_todo_result','aat_custom_style_1','aat_type_code',
			'users.name as assign','aat_type_name','ins_name','act_subcst')
			->get();
		}elseif(checkRule(array('STF'))) {
			$lead_data = Prs_accessrule::whereIn('slm_rules',['colaborator','master'])->where('slm_user',$user->id)->select('slm_lead')->get()->toArray();
			$lds_idr = array();
		...
		foreach ($colect_data as $key => $value) {
			...
			$title[$key] = $value->aat_type_name.' - '.$value->ins_name;
			$data[$key] =[
				'id' => $value->act_id, 
	...
	public function sourceDataTiketCalender(Request $request)
	{
		...
		if (checkRule(array('ADM','AGM','MGR.PAS'))) {
			...
			->select('act_id','ins_name','aat_type_button','aat_custom_class_1','act_activities.act_task_times_due','act_todo_result','aat_custom_style_1','aat_type_code',
			'users.name as assign','aat_type_name')
			->get();
		}elseif(checkRule(array('MGR'))) {
			$lead_data = Prs_accessrule::whereIn('slm_rules',['colaborator','master','manager'])->where('slm_user',$user->id)->select('slm_lead')->get()->toArray();
			$lds_idr = array();
	...
	public function sourceDataActivityCalenderSubCst(Request $request)
	{
		$id = $request->cst_id;
		$user = Auth::user();
		$start_date_arr = explode(" ", $request->start);
		...
		if (checkRule(array('ADM', 'AGM', 'MGR.PAS'))) {
			...
			->select(
				...
				'aat_type_button',
				'aat_custom_class_1',
				'act_activities.act_task_times_due',
				'act_todo_result',
				'aat_custom_style_1',
				'aat_type_code',
				'users.name as assign',
				'aat_type_name',
				'cst_name'
			)
			->get();
	...
	public function sourceDataActivityDetailCalender(Request $request)
	{
		...
		->select('act_id','ins_name','act_subcst','act_activities.act_task_times_due','act_todo_result','act_todo_describe','users.name as assign','aat_type_name','act_run_status','act_user_teams','act_user_customer')
		->first();
		...
		$result = [
			'param'=>true,
			'id'=> $colect_data->act_id,
			'data_act_name' => $colect_data->ins_name,
			'data_act_subname' => getNameSubcustomer($colect_data->act_subcst),
	...
	public function storeActivitiesNew(Request $request)
	{
		...
		$data = [
			...
			"act_subcst" => $request->sub_customer,
	...
	public function storeCloseActivities(Request $request)
	{
		...
		$data = [
			...
			"act_cst" => $request->customer,
			"act_subcst" => $request->subcustomer,
			"act_todo_type_id" => $request->action_todo,
			"act_run_status" => $act_run_status,
			'act_todo_priority' => null,
			'act_user_customer' => $pic_user,
  ...
  public function storeActivitiesLead(Request $request)
  {
	...
	public function viewTicketActivity(Request $request)
	{
		...
		if (checkRule(array('ADM','AGM','MGR.PAS'))) {
			...
			$lds_ids = array_unique($lds_idr);
			$all_activities = Act_activity::join('act_activity_types','act_activities.act_todo_type_id','=','act_activity_types.aat_id')
			->leftjoin('cst_institutions', 'act_activities.act_subcst','=', 'cst_institutions.ins_id')
			->where('act_label_category', 'TICKET')
			->whereIn('act_activities.act_lead_id',$lds_ids)
			->select('act_id','aat_type_code', 'act_subcst','act_cst')
			->get();
		} elseif(checkRule(array('STF'))) {
			$lead_data = Prs_accessrule::whereIn('slm_rules',['colaborator','master'])->where('slm_user',$user->id)->select('slm_lead')->get()->toArray();
			$lds_idr = array();
		...
		$customer_all = Cst_institution::select('ins_id','ins_name')->get();
