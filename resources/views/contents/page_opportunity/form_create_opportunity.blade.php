			...
			<form id="formContent1" enctype="multipart/form-data" action="javascript:void(0)" method="Get" autocomplete="off">
				...
				<div class="card-body card-body-custom">
					<div class="row mb-2">
						<div class="col-xl-6">
							<div class="mb-3 row">
								<label class="col-4 col-form-label custom-label" style="text-align: right;">Select Customer</label>
							...
							<div class="mb-3 row" id="job-position-area">
								<label class="col-4 col-form-label custom-label">Project Name</label>
								<div class="col">
									<select type="text" class="form-select" name="lead_id" id="select-lead" value="" placeholder="Select lead project here">
										<option value="{{ null }}"></option>
									</select>
								</div>
							</div>
...
<script>
	...
	var select_lead = new TomSelect("#select-lead",{
		create: false,
		maxItems: 1,
		valueField: 'id',
		labelField: 'title',
		...
		render: {
			...
			item: function(data, escape) {
				return '<div id="select-lead">'+escape(data.title)+'</div>';
			}
		}
	});
...
$(document).ready(function() {
	$("#formContent1").submit(function(e) {
		...
		$.ajax({
			type: "POST",
			url: "{{ route('store-new-opportunity') }}",
			data: formData1,
			cache: false,
			contentType: false,
			processData: false,
