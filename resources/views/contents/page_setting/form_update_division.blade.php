...
@section('pagetitle')
	...
	<h4 class="page-title">Setting: Division</h4>
@endsection
@section('breadcrumb')
	...
	<div class="col-md-12 ">
		<div class="card" style="margin-bottom:150px;">
			<div class="card-header card-header-custom card-header-light">
				<h3 class="card-title">Form Update Division</h3>
			...
			<form id="formContent1" enctype="multipart/form-data" action="javascript:void(0)" method="Get" autocomplete="off">
				@csrf
				<input type="hidden" name="id_division" value="{{ $division->div_id }}">
				<div class="card-body card-body-custom">
					<div class="row mb-2">
						<div class="col-xl-10">
							<div class="mb-3 row">
								<label class="col-4 col-form-label custom-label" style="text-align: right;">Division Name</label>
								<div id="select-customer-area" class="col">
									<input name="division" id="id-division" type="text" class="form-control" value="{{ $division->div_name }}">
								</div>
							</div>
						</div>
						<div class="col-xl-2">
...
$(document).ready(function() {
	$("#formContent1").submit(function(e) {
		...
		$.ajax({
			type: "POST",
			url: "{{ route('store-update-division') }}",
			data: formData1,
			cache: false,
			contentType: false,
			processData: false,
					...
					$.confirm({
						...
						buttons: {
							goToLead:{
								text:"Go To Division",
								action:function () {  
									window.location.href = "{{ url('setting/division') }}/";
								}
							}
						}
					});
