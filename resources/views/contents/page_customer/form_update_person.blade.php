		...
		<div class="card" style="margin-bottom:150px;">
			<div class="card-header card-header-custom card-header-light">
				...
				<div class="card-actions" style="padding-right: 10px;">
					...
					<a href="{{ url('customer/detail-customer/'.$dataperson->cnt_cst_id.'?extpg=information') }}">
			...
			<form id="formContent1" enctype="multipart/form-data" action="javascript:void(0)" method="post" autocomplete="off">
				...
				<div class="card-body card-body-custom">
					...
					<div class="row">
						...
						<div class="col-xl-9">
							<div class="mb-3 row" id="institution-name-area">
								<label class="col-3 col-form-label custom-label">Person Name</label>
								<div class="col">
									<input name="institution_name" id="institution-name" type="text" class="form-control" aria-describedby="" placeholder="" value="{{ $dataperson->cnt_fullname }}" >
								</div>
							</div>
							<div class="mb-3 row" id="institution-category-area">
								<label class="col-3 col-form-label custom-label">Institution Names</label>
								<div class="col">
									<select type="text" class="form-select select-categories" name="business_category" id="select-category" value="">
										<option value="{{ $dataperson->cst_name }}">{{ $dataperson->cst_name }}</option>
										@foreach ($company as $item)
										<option value="{{ $item->cst_name }}">{{ $item->cst_name }}</option>
										@endforeach
									</select>
								</div>
							</div>
					...
					<div class="row">
						<div class="col-xl-6">
							<div class="mb-3 row" id="job-position-area" style="">
								<label class="col-3 col-form-label custom-label">Job Position</label>
								<div class="col">
									<input name="job_position" id="job-position" type="text" class="form-control" placeholder="Type job position here" value="{{ $dataperson->cnt_company_position }}">
								</div>
							</div>
							<div class="row">
				...
				<div class="card" style="border: none;">
						...
						<div class="tab-content">
							<div class="tab-pane @if ($a == 'tab3') active show @endif" id="tabs-notes" role="tabpanel">
								<div class="mb-3">
									...
									<textarea id="notesInput">{{ $dataperson->cnt_notes }}</textarea>
								</div>
							</div>
                  ...
                  <div class="form-label">Setting for data view customrt</div>
									<select type="text" class="form-select val-input-option mb-2" name="view_option" placeholder="Select lead status for now ..." id="select-option-view" value="">
										<option value="{{ $dataperson->view_option_individu }}">{{ Str::of($dataperson->view_option_individu)->title() }}</option>
										<option value="MODERATE">Moderate</option>
										<option value="PRIVATE">Private</option>
										<option value="PUBLIC">Public</option>
									</select>
									<span id="opt_public" @if ($dataperson->view_option == 'PUBLIC') style="display: true;" @else style="display: none;" @endif >
									...
									<span id="opt_moderate" @if ($dataperson->view_option == 'MODERATE') style="display: true;" @else style="display: none;" @endif>
									...
									<span id="opt_private" @if ($dataperson->view_option == 'PRIVATE') style="display: true;" @else style="display: none;" @endif>
...
<script type="text/javascript">
	...
	var x = tinymce.init({
		...
		plugins: [
			'advlist', 'autolink', 'lists', 'link', 'image', 'charmap', 'preview', 'searchreplace', 'visualblocks', 'advcode', 'fullscreen',
	...
	<script>
		$(document).ready(function() {
			$('#formContent1').submit(function(e) {
				...
				$.ajax({
					type: 'POST',
					url: "{{ route('store-update-personal',['id'=>$id]) }}",
					data: formData,
					cache: false,
					contentType: false,
					processData: false,
