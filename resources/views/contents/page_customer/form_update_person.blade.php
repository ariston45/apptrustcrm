		...
		<div class="card" style="margin-bottom:150px;">
			<div class="card-header card-header-custom card-header-light">
				...
				<div class="card-actions" style="padding-right: 10px;">
					...
					<a href="{{ url('customer/contacts/'.$company->ins_id) }}">
					...
					<div class="row mb-2">
						...
						<div class="col-xl-9">
							...
							<div class="mb-3" id="person-name-area">
								...
								<input name="person_name" id="person-name" type="text" class="form-control" aria-describedby="emailHelp" placeholder="Name of person" value="{{ $dataperson->cnt_fullname }}" required>
								<input type="hidden" name="cst_id" value="{{ $dataperson->cnt_cst_id }}" >
								<input type="hidden" name="person_id" value="{{ $dataperson->cnt_id }}" >
								<input type="hidden" name="loc_id" @if ($location) value="{{ $location->cnt_id }}" @else value="" @endif>
					...
					<div class="row mb-3">
						<div class="col-xl-6">
								...
								<div class="col">
									<select type="text" class="form-select " name="sub_customer" placeholder="Select sub customer" id="select-subcustomer" value="" required>
										...
										@foreach ($subcustomer as $list)
											<option value="{{ $list->cst_id }}" @if ($list->cst_id == $dataperson->cnt_subcst_id) selected @endif >{{ $list->cst_name }}</option>
										@endforeach
									</select>
								</div>
							</div>
							<div class="mb-3 row" id="job-position-area">
								...
								<div class="col">
									<input name="job_position" id="job-position" type="text" class="form-control" placeholder="Type job position here" value="{{ $dataperson->cnt_company_position }}" required>
								</div>
							</div>
							<div class="mb-3 row">
								<label class="col-3 col-form-label custom-label">Mobile</label>
								<div id="multiInputMobile" class="col">
									@if (count($mobile) == 0)
										<div class="input-group">
											<input name="mobile[]" id="mobile" type="text" class="form-control" placeholder="Type mobile number or whatsapp number here" value="">
										...
										@foreach ($mobile as $key => $list)
											@if ($key == 0)
												<div class="input-group mt-2">
													<input name="mobile[]" id="mobile" type="text" class="form-control" placeholder="Type mobile number or whatsapp number here" value="{{ $list }}">
													<button id="add-button-one" class="btn btn-addons" type="button"><i class="ri-add-line custom-icon-min"></i></button>
												</div>
											@else
												<div class="input-group mt-2">
								...
								<div id="multiInputEmail" class="col">
									...
									@else
										@foreach ($email as $key => $list)
											@if ($key == 0)
												<div class="input-group">
													<input name="email[]" id="email" type="email" class="form-control" placeholder="Type email address here" value="{{ $list }}">
													<button id="add-button-three" class="btn btn-addons" type="button"><i class="ri-add-line custom-icon-min"></i></button>
												</div>
											@else
												<div class="input-group">
							...
							<div class="mb-3 row">
								...
								<div class="col">
									<textarea rows="5" class="form-control" name="note">{{ $dataperson->cnt_notes }}</textarea>
								</div>
							</div>
						</div>
						<div class="col-xl-6">
								...
								<div class="col">
									<select type="text" class="form-select mb-2 " name="addr_province" placeholder="Select province" id="addr-province" value="">
										...
										@if ($location)
											...
											<option value="{{ $list->prov_name }}" @if ($list->prov_name == $location->loc_province) selected @endif>{{ $list->prov_name }}</option>
											@endforeach
										@else
											@foreach ($provincies as $list)
									...
									@if ($location)
										<input id="addr-street" name="addr_street" type="text" class="form-control mb-2" placeholder="Street" autocomplete="off" value="{{ $location->loc_street }}">
									@endif
								</div>
							</div>
						</div>
